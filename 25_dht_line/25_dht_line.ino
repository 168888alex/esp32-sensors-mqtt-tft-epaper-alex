...
#error Bluetooth is not enabled for this ESP32 core
#endif

#if !defined(CONFIG_BT_SPP_ENABLED)
#error Bluetooth SPP is not available for this ESP32 target
#endif

BluetoothSerial SerialBT;
const char *BLUETOOTH_DEVICE_NAME = "A0116";
const char *BLUETOOTH_PIN = "0962";
const char *WIFI_SSID = "YOUR_WIFI_SSID";
const char *WIFI_PASSWORD = "YOUR_WIFI_PASSWORD";
const char *GOOGLE_SCRIPT_ID = "AKfycbyR-Yp-uu4nIvnjvnkILaQ5AX8yFxp-UpBO-Sqs0su3ai1N_BvQsz_Q";
const char *GOOGLE_SHEET_ID = "12KsLiWQTnof__i7YvyV-SjD906IyPe1kT2PzY_eZSXA";
...
const unsigned long LINE_ALERT_INTERVAL_MS = 10000;
const unsigned long UPLOAD_INTERVAL_MS = 10000;
unsigned long lastUploadMillis = 0;
unsigned long lastLineAlertMillis = 0;
...
void showLineStatus(const char *status) {
  oledTextFixed(0, 0, status, 12);
  Serial.println(status);
}

bool sendLineAlert(int temperature, int humidity, int lightPercent) {
  if (WiFi.status() != WL_CONNECTED && !connectWiFi()) {
    return false;
  }

  String message = String("異常警告：温度 ") + temperature + " C、湿度 " + humidity + " %、明るさ " + lightPercent + " %";
  String payload = String("{\"to\":\"") + LINE_USER_ID
                 + "\",\"messages\":[{\"type\":\"text\",\"text\":\"" + message + "\"}]}";

  WiFiClientSecure lineClient;
  lineClient.setInsecure();
  lineClient.setTimeout(8000);
  showLineStatus("LINE SENDING");
  if (!lineClient.connect("api.line.me", 443)) {
    Serial.println("LINE connection failed");
    showLineStatus("LINE FAIL");
    delay(700);
    return false;
  }
  ...
  Serial.printf("LINE response: %s", statusLine.c_str());
  showLineStatus(success ? "LINE OK" : "LINE FAIL");
  delay(700);
  return success;
}
...
void handleBluetoothCommands() {
  while (SerialBT.available()) {
    ...
    if (command.equalsIgnoreCase("STATUS")) {
      SerialBT.println("Commands: STATUS, WIFI, GOOGLE, LINE");
      SerialBT.printf("WiFi: %s\n", WiFi.status() == WL_CONNECTED ? "CONNECTED" : "DISCONNECTED");
    } else if (command.equalsIgnoreCase("WIFI")) {
      SerialBT.printf("WiFi: %s, IP: %s\n", WiFi.status() == WL_CONNECTED ? "CONNECTED" : "DISCONNECTED", WiFi.localIP().toString().c_str());
    } else if (command.equalsIgnoreCase("GOOGLE")) {
      SerialBT.println("Google Sheet upload interval: 10 seconds");
    } else if (command.equalsIgnoreCase("LINE")) {
      SerialBT.println("LINE alert: temperature >= 32 C or humidity >= 80 %, interval 10 seconds");
    } else if (command.length() > 0) {
      SerialBT.println("Unknown command. Use STATUS, WIFI, GOOGLE or LINE");
    }
  }
}
void setup() {
  ...
  Serial.printf("Bluetooth started: %s, PIN: %s\n", BLUETOOTH_DEVICE_NAME, BLUETOOTH_PIN);
...
void loop() {
  ...
  if (error == SimpleDHTErrSuccess) {
    ...
    SerialBT.printf("Temperature: %d C, Humidity: %d %%, Light: %d %%\n", temperature, humidity, lightPercent);

    bool abnormal = temperature >= 32 || humidity >= 80;
    if (!abnormal) {
      lastLineAlertMillis = 0;
    } else if (lastLineAlertMillis == 0 || millis() - lastLineAlertMillis >= LINE_ALERT_INTERVAL_MS) {
      bool sent = sendLineAlert(temperature, humidity, lightPercent);
      lastLineAlertMillis = millis();
      Serial.println(sent ? "LINE alert sent" : "LINE alert failed");
      oledTextFixed(0, 0, "ENVIRONMENT", 12);
    }
