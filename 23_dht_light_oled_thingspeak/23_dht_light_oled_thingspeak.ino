...
bool sendToThingSpeak(int temperature, int humidity, int lightPercent) {
  ...
  if (!http.begin(client, url)) {
    Serial.println("ThingSpeak connection setup failed");
    return false;
  }
  ...
  http.end();
  Serial.printf("ThingSpeak response: %d, entry: %s\n", responseCode, response.c_str());
  return responseCode == HTTP_CODE_OK && response != "0";
}
...
void loop() {
  ...
  if (error == SimpleDHTErrSuccess) {
    ...
    if (lastUploadMillis == 0 || millis() - lastUploadMillis >= UPLOAD_INTERVAL_MS) {
      bool uploaded = sendToThingSpeak(temperature, humidity, lightPercent);
