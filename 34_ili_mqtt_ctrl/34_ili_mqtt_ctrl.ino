...
void drawStatusBar() {
  ...
  tft.drawText(146, 201, mqttClient.connected() ? "O" : "X", mqttClient.connected() ? UI_GREEN : UI_RED);
}
...
void loop() {
  ...
  if (now - lastClockUpdate >= CLOCK_UPDATE_INTERVAL) {     drawDateTime();     lastClockUpdate = now;   }    delay(50);
}
